...
css = concat
  ...
  , "body { font-family: sans-serif; margin: 1.5em auto; max-width: 960px;"
  , "       background: #fafafa; color: #222; font-size: 0.94em; }"
  , "h1 { font-size: 1.4em; margin-bottom: 0.2em; }"
  , "p  { color: #555; margin: 0.3em 0 0.7em; }"
  , ".legend { display: flex; gap: 1em; align-items: center; margin: 0.8em 0 1.2em;"
  , "          padding: 0.5em 1em; background: #f0f0f0; border-radius: 4px;"
  ...
  , ".card.aps    > .card-hdr { background: #d4edda; border-bottom: 2px solid #28a745; }"
  , ".card.preaps > .card-hdr { background: #d1ecf1; border-bottom: 2px solid #17a2b8; }"
  , ".card.noaps  > .card-hdr { background: #f0f0f0; border-bottom: 2px solid #aaa; }"
  , ".card-hdr { display: flex; align-items: baseline; gap: 0.7em; padding: 0.5em 0.9em; }"
  , ".mname { font-size: 1.1em; font-weight: bold; color: #111; }"
  , ".carrier { font-size: 0.85em; color: #666; }"
  , ".badge { padding: 0.1em 0.6em; border-radius: 3px; font-size: 0.78em;"
  , "         font-weight: bold; margin-left: auto; }"
  , ".badge.aps    { background: #28a745; color: white; }"
  , ".badge.preaps { background: #17a2b8; color: white; }"
  , ".badge.noaps  { background: #aaa;    color: white; }"
  , ".desc { padding: 0.4em 0.9em 0; font-size: 0.9em; color: #444; margin: 0; }"
  , ".body { display: flex; gap: 1.5em; padding: 0.7em 0.9em 0.8em; flex-wrap: wrap; }"
  , ".tbl-wrap { flex: 0 0 auto; }"
  , ".op-tbl { border-collapse: collapse; font-size: 0.88em; }"
  , ".op-tbl th, .op-tbl td { border: 1px solid #ddd; padding: 3px 8px; text-align: center;"
  , "                         min-width: 2em; }"
  , ".op-tbl th { background: #f5f5f5; font-weight: normal; font-style: italic; }"
  , ".props-wrap { flex: 1 1 200px; }"
  , ".props { display: flex; flex-wrap: wrap; gap: 0.3em; align-content: flex-start; }"
  , ".chip { display: inline-block; padding: 0.15em 0.5em; border-radius: 3px;"
  , "        font-size: 0.8em; border: 1px solid; }"
  , ".chip.yes { background: #d4edda; border-color: #28a745; color: #155724; }"
  , ".chip.no  { background: #f8f8f8; border-color: #ccc;    color: #999; }"
  ]
